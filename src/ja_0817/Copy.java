...
public class Copy {
	public static void main(String[] args) {
		...
		if(args.length !=2) //매개변수 확인
			...
			System.err.println("Usage : Copy <sourcs> < destination > ");
	...
	public static void copy(String from_name , String to_name)throws IOException{
		...
		//to name 이 디렉토리이면 , source 파일 이름을 destination 파일 이름으로 그대로 사용
		...
		if(to_file.exists()) {
			...
			//예외가 발생하였더라도 스트림을 항상 닫아 주어야함
			
			finally {
				if(from != null)	//	객체가 존재 한다면 ( null 이 아니라면 )
					try {
						from.close();
					}catch (IOException e) {  ;  }
				if(to != null)
